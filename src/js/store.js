...
App.Store = (function(){
  ...
  function addEpisode(ep) { snapshot(); project.episodes.push(ep); project.episodes.sort((a,b)=>a.order-b.order); emit('change',{type:'addEp',targetId:ep.id,targetName:ep.title||''}); }
  function updateEpisode(id, data) { snapshot(); const ep=getEpisode(id); if(ep) Object.assign(ep,data); emit('change',{type:'updateEp',targetId:id,targetName:(ep&&ep.title)||''}); }
  function removeEpisode(id) { snapshot(); const _n=(getEpisode(id)||{}).title||''; project.episodes=project.episodes.filter(e=>e.id!==id); project.scenes=project.scenes.filter(s=>s.episodeId!==id); project.events=project.events.filter(e=>e.episodeId!==id); emit('change',{type:'removeEp',targetId:id,targetName:_n}); }
  ...
  function addScene(sc) { snapshot(); project.scenes.push(sc); emit('change',{type:'addScene',targetId:sc.id,targetName:sc.title||''}); }
  function updateScene(id, data) { snapshot(); const sc=getScene(id); if(sc) Object.assign(sc,data); emit('change',{type:'updateScene',targetId:id,targetName:(sc&&sc.title)||''}); }
  function removeScene(id) { snapshot(); const _n=(getScene(id)||{}).title||''; project.scenes=project.scenes.filter(s=>s.id!==id); project.events=project.events.filter(e=>e.sceneId!==id); emit('change',{type:'removeScene',targetId:id,targetName:_n}); }
  ...
  function addEvent(ev) { snapshot(); project.events.push(ev); emit('change',{type:'addEvent',targetId:ev.id,targetName:ev.title||''}); }
  function updateEvent(id, data) { snapshot(); const ev=getEvent(id); if(ev) Object.assign(ev,data); emit('change',{type:'updateEvent',targetId:id,targetName:(ev&&ev.title)||''}); }
  function removeEvent(id) { snapshot(); const _n=(getEvent(id)||{}).title||''; project.events=project.events.filter(e=>e.id!==id); project.connections=project.connections.filter(c=>c.from!==id&&c.to!==id); emit('change',{type:'removeEvent',targetId:id,targetName:_n}); }
  ...
  function addCharacter(ch) { snapshot(); project.characters.push(ch); emit('change',{type:'addChar',targetName:ch.name||''}); }
  function removeCharacter(id) { snapshot(); const _n=(project.characters.find(c=>c.id===id)||{}).name||''; project.characters=project.characters.filter(c=>c.id!==id); emit('change',{type:'removeChar',targetName:_n}); }

  // Category helpers
  function getCategories() { return project.categories; }
  function addCategory(key, data) { snapshot(); project.categories[key]=data; emit('change',{type:'addCat',targetName:key}); }
  function removeCategory(key) { snapshot(); delete project.categories[key]; emit('change',{type:'removeCat',targetName:key}); }
