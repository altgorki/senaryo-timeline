...
App.Export = (function(){
  ...
  var _fontCache = { regular: null, bold: null };
  ...
  function _loadFonts(callback) {
    if (_fontCache.regular) { callback(); return; }
    var done = 0;
    function check() { if (++done >= 2) callback(); }
    fetch('https://fonts.gstatic.com/s/courierprime/v11/u-450q2lgwslOqpF_6gQ8kELWwY.ttf')
      .then(function(r) { return r.arrayBuffer(); })
      .then(function(buf) { _fontCache.regular = _arrayBufferToBase64(buf); check(); })
      .catch(function() { check(); });
    fetch('https://fonts.gstatic.com/s/courierprime/v11/u-4k0q2lgwslOqpF_6gQ8kELY7pMf-c.ttf')
      .then(function(r) { return r.arrayBuffer(); })
      .then(function(buf) { _fontCache.bold = _arrayBufferToBase64(buf); check(); })
      .catch(function() { check(); });
  }

  function _registerFonts(doc) {
    if (_fontCache.regular) {
      doc.addFileToVFS('CourierPrime-Regular.ttf', _fontCache.regular);
      doc.addFont('CourierPrime-Regular.ttf', 'CourierPrime', 'normal');
    }
    if (_fontCache.bold) {
      doc.addFileToVFS('CourierPrime-Bold.ttf', _fontCache.bold);
      doc.addFont('CourierPrime-Bold.ttf', 'CourierPrime', 'bold');
    }
  }

  function _fontName() {
    return _fontCache.regular ? 'CourierPrime' : 'courier';
  }
